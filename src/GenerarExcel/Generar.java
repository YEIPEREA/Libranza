...
//String numero=Or.replace("Corte ", "").replace("_", "");
		...
		try {
			...
			String RT1 = "exec NumeroConvenios";
			...
			for (int L = 0; L < cantiidad; L++) {
				...
				String RT2 = "SELECT [CONVENIO],[NIT],[NOMBRE_EMPRESA],[FECHA_DE_PAGO],[Numero_de_Documento],[Nombre],[No_Obligación],[Nombre_Oficina],"
						...
						+ " FROM [Libranza].[dbo].[BASE_SALDOS_ACTUALIZADOS_FACTURACION_CORTE] WHERE  CONVENIO='"
						+ valor + "'  ORDER BY CAST(CONVENIO AS INT)";
				result2 = base.ConsultarQuery(RT2);
				int cantiidad2 = result2.getRowCount();
				valor2 = result2.getValueAt(0, 0).toString();

				String RT3 = "SELECT Documento,Regional,Contacto  FROM [Libranza].[dbo].[Parametrizados] where Convenio='"
						+ valor + "'";
...
//WritableCellFormat headerFormat = new WritableCellFormat();
					...
					if (Correo.equals("")) {
						LibroEX = Workbook.createWorkbook(new File(direccion + "\\OTROS\\" + valor + ".xls"), conf);// ubicacion
																							// archivo
					} else {
						LibroEX = Workbook.createWorkbook(new File(direccion + "\\" + valor3 + "\\" + valor + ".xls"),
								conf);// ubicacion de los archivos,nombree de la carpeta,nombre del archivo
					}

					WritableSheet Sheet = LibroEX.createSheet("Tabla", 0);// nombre de la hoja del excel y la posicion
					WritableFont h = new WritableFont(WritableFont.TAHOMA, 9, WritableFont.NO_BOLD);// tipo de letra y
																									// tamaño
					WritableFont h2 = new WritableFont(WritableFont.TAHOMA, 9, WritableFont.BOLD);// tipo de letra y
																									// tamaño
					WritableFont h3 = new WritableFont(WritableFont.TAHOMA, 9, WritableFont.NO_BOLD);// tipo de letra y
																										// tamaño
					...
					String[] Encabezado = new String[] { // estructura del archivo excel "Títulos"
							"Convenio", "Nit", "Nombre Empresa", "Fecha de Pago", "Número de Documento", "Nombre",
							"No Obligación", "Nombre Oficina", "Cuotas Pactadas", "Cuotas a Pagar", "Vr Cuota Mensual",
							"Vr en Mora", "Vr Total a Pagar", "Vr Intereses en Mora", "Vr Pago Empresa", "Novedad",
							"Observaciones" };
...
//valor2=valor2.replace(".", ",");
							...
							if (j == 0 || j == 1 || j == 4 || j == 8 || j == 9 || j == 10 || j == 11 || j == 12
									|| j == 13 || j == 14) {
								...
								if (j == 14 && numeroLong == 0) {// en caso de encontrar un valor de pago empresa igual
																	// acero
					...
					// total Vr cuota mensual
					Sheet.addCell(new jxl.write.Number(10, (cantiidad2 + 2), numeroLong10, hFormat3));
					// total Vr en mora
					Sheet.addCell(new jxl.write.Number(11, (cantiidad2 + 2), numeroLong11, hFormat3));
					// total Vr total a pagar
					Sheet.addCell(new jxl.write.Number(12, (cantiidad2 + 2), numeroLong12, hFormat3));
					// total Vr intereses en mora
					Sheet.addCell(new jxl.write.Number(13, (cantiidad2 + 2), numeroLong13, hFormat3));
					// cantidad de registros
					Sheet.addCell(new jxl.write.Number(0, (cantiidad2 + 2), cantiidad2, hFormat));
...
//funcion.EcriptarZip(direccion,valor,"OTROS",valor4);
						...
						if (Porcentaje >= (cont + 1)) {
							LabelPorcentaje.setText(Porcentaje + "% " + " Van " + n + " de " + cantiidad
									+ " Generando Archivo " + valor + " " + valor3);
							PanelPorcentaje.update(PanelPorcentaje.getGraphics());
							cont++;
							if (cont > 99) {
								cont = 99;
...
//**********************************************************************************************************
	...
	public void BaseConsolidado(String direccion, JPanel Panel, JLabel Label) {
		...
		try {
			...
			conf.setEncoding("ISO-8859-1");
			WritableWorkbook LibroEX;
			LibroEX = Workbook.createWorkbook(new File(direccion + "\\BASE_CONSOLIDADA_CLIENTES_LIBRANZA.xls"), conf);// ubicacion
																														// de
																														// los
																														// archivos,nombree
																														// de
			...
			String[] Encabezado = new String[] { "CONVENIO", "NIT", "NOMBRE_EMPRESA", "FECHA_DE_PAGO",
					"correo_electronico_Empresa", "ITEM", "Tipo_de_Documento", "Numero_de_Documento", "Nombre",
					"No_Obligación", "Código_Oficina", "Nombre_Oficina", "Vr_Cuota_Mensual", "Vr_en_Mora",
					"Vr_Total_a_Pagar", "Vr_Intereses_en_Mora", "Cuotas_Pactadas", "Cuotas_a_Pagar",
					"Valor_Pago_Empresa", "novedad", "Observaciones" };
				...
				for (int j = 0; j < Encabezado.length; j++) {// columnas
					...
					if (j == 0 || j == 1 || j == 7 || j == 9 || j == 10 || j == 12 || j == 13 || j == 14 || j == 15) {
						...
						if (j == 9) {
							...
							Sheet.addCell(new jxl.write.Number(j, (L + 1), numeroLong, hFormat));
						}
					} else {
...
//**********************************************************************************************************
	...
	public void FacturacionCorte(String direccion, JPanel Panel, JLabel Label) {
		...
		try {
			...
			conf.setEncoding("ISO-8859-1");
			WritableWorkbook LibroEX;
			LibroEX = Workbook.createWorkbook(new File(direccion + "\\BASE_SALDOS_ACTUALIZADOS_FACTURACION_CORTE.xls"),
					conf);// ubicacion de los archivos,nombree de la carpeta,nombre del archivo
			...
			String[] Encabezado = new String[] { "CONVENIO", "NIT", "NOMBRE_EMPRESA", "FECHA_DE_PAGO",
					"Correo_electronico_Empresa", "ITEM", "Tipo_de_Documento", "Numero_de_Documento", "Nombre",
					"Obligación", "LLAVE", "No_Obligación", "Código_Oficina", "Nombre_Oficina", "Vr_Cuota_Mensual",
					"Vr_en_Mora", "Vr_Total_a_Pagar", "Vr_Intereses_en_Mora", "Cuotas_Pactadas", "Cuotas_a_Pagar",
					"Valor_Pago_Empresa", "novedad", "Observaciones", "mo_dias_vencido_op", "mo_fecha_prox_vencimiento",
					"mo_saldo_cuota_prox_venc", "mo_saldo_capital_vigente", "mo_saldo_capital_vencido",
					"mo_saldo_interes_vigente", "mo_saldo_interes_vencido", "mo_saldo_interes_contingente",
					"mo_saldo_mora_contingente", "mo_saldo_seguro_vida_vigente", "mo_saldo_seguro_vida_vencido",
					"mo_saldo_otros_vigente", "mo_saldo_otros_vencidos", "mo_estado_obligacion", "mo_fecha_ult_pago",
					"mo_valor_ult_pago", "mo_numero_cuotas_vencidas" };
			...
			for (int L = 0; L < cantiidad; L++) {
				...
				for (int j = 0; j < Encabezado.length; j++) {// columnas
					...
					if (j == 0 || j == 1 || j == 7 || j == 11 || j == 15 || j == 16 || j == 17 || j == 25 || j == 26
							|| j == 27 || j == 28 || j == 29 || j == 30 || j == 31 || j == 32 || j == 33 || j == 34
							|| j == 35 || j == 38 || j == 39) {
						numEntero = Double.parseDouble(valor2);
						numeroLong = Math.round(numEntero);
						if (j == 11) {
...
//========================================================================================================
			for (int A = 0; A < cantiidad; A++) {
				...
				if (Regional.contains("DIRECCION GENERAL") == true) {
					Regional = "BOGOTA";
				}
				...
				String RT2 = "UPDATE Libranza.dbo.NO_COINCIDENTE2 set DirenciaCvsB=0 where DirenciaCvsB is null "
						...
						+ " VrPEmpresa,Novedad,Observaciones,CMensual,DirenciaCvsB from Libranza.dbo.NO_COINCIDENTE2 where cod="
						+ Convenio;
				DefaultTableModel result2;
				result2 = base.ConsultarQuery(RT2);
				...
				if (cantiidad2 > 0) {
					...
					conf.setEncoding("ISO-8859-1");
					WritableWorkbook LibroEX;
					LibroEX = Workbook.createWorkbook(new File(direccion + "\\" + Regional + "\\" + Convenio2 + ".xls"),
							conf);// ubicacion del archivo nombre del archivo
					...
					String[] Encabezado = new String[] { "CONVENIO ", "NIT ", "NOMBRE EMPRESA ", "FECHA DE PAGO ",
							"Número de documento ", "Nombre", "Obligación ", "Nombre Oficina ", "Cuotas pactadas ",
							"Cuotas a pagar ", "Vr cuota mensual ", "Vr en mora ", "Vr total a pagar ",
							"Vr intereses en mora ", "Valor Pago Empresa ", "Novedad ", "Observaciones ", "FACTURACION",
							"CATRADIA"
							/*
...
//System.out.println("TABLA : "+valor2);
							...
							if (j == 1 || j == 4 || j == 6 || j == 10 || j == 11 || j == 12 || j == 13 || j == 17
									|| j == 18 || j == 8 || j == 9) {
								numEntero = Double.parseDouble(valor2);
								numeroLong = Math.round(numEntero);
								if (j == 10 || j == 11 || j == 12 || j == 13 || j == 17 || j == 18) {
									...
									if (j == 17) {
										suma1 = numeroLong + suma1;
									} // suma de Facturacion vs bancolomba
									else if (j == 18) {
										suma2 = numeroLong + suma2;
									} // suma de catradia vs bancolombia
								} else {
						...
						if (Porcentaje >= (cont + 1)) {
							LabelPorcentaje.setText(
									"Generando Archivo " + Porcentaje + "% " + " Van " + n + " de " + cantiidad);
							PanelPorcentaje.update(PanelPorcentaje.getGraphics());
							if (Porcentaje > cont) {
								cont = Porcentaje;
							}
...
//========================================================================================================
			...
			conf.setEncoding("ISO-8859-1");
			WritableWorkbook LibroEX;
			LibroEX = Workbook.createWorkbook(new File(direccion + "\\COINCIDENTE.xls"), conf);// ubicacion del archivo
																								// nombre del archivo
			...
			WritableCellFormat hFormat = new WritableCellFormat(h);// para mandar el formato
			WritableCellFormat hFormat2 = new WritableCellFormat(h2);// para mandar el formato
			WritableCellFormat hFormat3 = new WritableCellFormat(h3, new NumberFormat("$_#,##0.00"));// formato tipo
																										// moneda
			WritableCellFormat hFormat4 = new WritableCellFormat(h3, new NumberFormat("###0"));// formato numerico

			String[] Encabezado = new String[] { "REGIONAL", "NUMERO CONVENIO",
					"FORMA DE PAGO (Nit Base Bancolombia - Consecutivo ", "FECHA DE PAGO", "IDENTIFICACION CLIENTE",
					"NOMBRE CLIENTE", "OBLIGACION ACTIVA", "VALOR PAGO EMPRESA", "OBSERVACION", };
			WritableCellFormat ColorFormato = new WritableCellFormat(hFormat2);
...
//valor2=valor2.replace(".", ",");
				...
				if (Porcentaje >= (cont + 1)) {
					LabelPorcentaje.setText(
							"Generando archivo coincidente " + Porcentaje + "% " + " Van " + n + " de " + cantiidad);
					PanelPorcentaje.update(PanelPorcentaje.getGraphics());
					if (Porcentaje > cont) {
						cont = Porcentaje;
					}
